...
export default async function ActualsPage({ params }: Props) {
  ...
  const { data } = await supabase
    .from('cc_actuals').select('*').eq('project_id', projectId).order('cost_date', { ascending: false })
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {canWrite && <Button asChild><Link href={`/projects/${projectId}/cost-control/actuals/new`}><Plus className="w-4 h-4 mr-2" />Add Actual</Link></Button>}
      </div>
